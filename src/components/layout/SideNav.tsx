import { useLocation, useNavigate } from 'react-router-dom'
import { useStore } from '../../store'
import { useAuthStore } from '../../store/authStore'
import { isSupabaseEnabled } from '../../lib/supabase'

interface NavItem {
  path: string
  label: string
  icon: React.ReactNode
  matchPrefix?: string
}

const mainItems: NavItem[] = [
  {
    path: '/home',
    label: 'ダッシュボード',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
      </svg>
    ),
  },
  {
    path: '/products',
    label: '商品管理',
    matchPrefix: '/products',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 10V7" />
      </svg>
    ),
  },
  {
    path: '/documents',
    label: '発注書',
    matchPrefix: '/documents',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
    ),
  },
]

const bottomItems: NavItem[] = [
  {
    path: '/settings',
    label: '設定',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
  },
]

interface Props {
  onClose?: () => void
}

export function SideNav({ onClose }: Props) {
  const location = useLocation()
  const navigate = useNavigate()
  const settings = useStore((s) => s.settings)
  const { currentUser, logout } = useAuthStore()

  const isActive = (item: NavItem) => {
    if (item.matchPrefix) return location.pathname.startsWith(item.matchPrefix)
    return location.pathname === item.path
  }

  const handleNav = (path: string) => {
    navigate(path)
    onClose?.()
  }

  return (
    <aside className="hidden md:flex flex-col w-60 bg-white border-r border-slate-200 min-h-svh sticky top-0 shrink-0">
      {/* Logo */}
      <div className="px-5 py-4 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-md bg-brand-600 flex items-center justify-center text-white font-bold text-xs shrink-0">
            M
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-slate-900 text-sm leading-tight">MySheet</div>
            <div className="text-xs text-slate-400 truncate">{settings.companyName || '屋号未設定'}</div>
          </div>
        </div>
      </div>

      {/* Main navigation */}
      <nav className="flex-1 px-3 py-4">
        <div className="text-xs font-medium text-slate-400 px-3 mb-2">メニュー</div>
        <div className="space-y-0.5">
          {mainItems.map((item) => {
            const active = isActive(item)
            return (
              <button
                key={item.path}
                type="button"
                onClick={() => handleNav(item.path)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors text-left
                  ${active
                    ? 'bg-brand-50 text-brand-700 font-medium'
                    : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                  }`}
              >
                <span className={active ? 'text-brand-600' : 'text-slate-400'}>
                  {item.icon}
                </span>
                {item.label}
              </button>
            )
          })}
        </div>

        <div className="mt-6">
          <div className="text-xs font-medium text-slate-400 px-3 mb-2">ツール</div>
          <button
            type="button"
            onClick={() => handleNav('/onboarding/1')}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-colors text-left"
          >
            <span className="text-slate-400">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
            </span>
            収益シミュレーション
          </button>
          <button
            type="button"
            onClick={() => handleNav('/preview/web-dashboard')}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors text-left
              ${location.pathname === '/preview/web-dashboard'
                ? 'bg-brand-50 text-brand-700 font-medium'
                : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
              }`}
          >
            <span className={location.pathname === '/preview/web-dashboard' ? 'text-brand-500' : 'text-slate-300'}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
            </span>
            <span className="flex-1">UIプレビュー</span>
            <span className="text-[10px] font-medium bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded">β</span>
          </button>
        </div>
      </nav>

      {/* Bottom section */}
      <div className="border-t border-slate-100">
        <div className="px-3 py-2">
          {bottomItems.map((item) => {
            const active = isActive(item)
            return (
              <button
                key={item.path}
                type="button"
                onClick={() => handleNav(item.path)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors text-left
                  ${active
                    ? 'bg-brand-50 text-brand-700 font-medium'
                    : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                  }`}
              >
                <span className={active ? 'text-brand-600' : 'text-slate-400'}>
                  {item.icon}
                </span>
                {item.label}
              </button>
            )
          })}
        </div>

        {/* User info */}
        <div className="px-4 py-3 border-t border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center text-brand-700 text-xs font-semibold shrink-0">
              {(currentUser?.user_metadata?.display_name || settings.displayName || 'U').charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium text-slate-900 truncate">
                {currentUser?.user_metadata?.display_name || settings.displayName || 'ユーザー'}
              </div>
              <div className="text-xs text-slate-400 truncate">
                {currentUser?.email || settings.email || ''}
              </div>
            </div>
            {isSupabaseEnabled && (
              <button
                type="button"
                onClick={() => logout()}
                title="ログアウト"
                className="text-slate-400 hover:text-slate-600 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
              </button>
            )}
          </div>
        </div>
      </div>
    </aside>
  )
}
