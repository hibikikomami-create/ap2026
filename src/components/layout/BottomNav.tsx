...
const items = [
  {
    ...
    matchPrefix: '/home',
    icon: (active: boolean) => (
      <svg className="w-5 h-5" fill={active ? 'none' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.2 : 1.8}>
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center justify-center flex-1 gap-0.5 select-none transition-transform active:scale-95"
            >
              <div className={`p-1.5 rounded-xl transition-all duration-200 ${active ? 'bg-brand-100' : ''}`}>
                <span className={`transition-colors duration-200 ${active ? 'text-brand-600' : 'text-slate-400'}`}>
                  {item.icon(active)}
                </span>
              </div>
